...
// y guarda el pedido pendiente en Google Sheets
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const baseUrl = process.env.BASE_URL || 'https://moto7.vercel.app';
    ...
    // Guardar pedido pendiente en Google Sheets
    await saveToSheets({
      orderId,
      estado: 'PENDIENTE',
  ...
  try {
    ...
    if (sheet.rowCount === 1 && sheet.columnCount === 1) {
      await sheet.setHeaderRow([
        'Orden ID', 'Estado', 'Nombre', 'DNI', 'Teléfono', 'Email',
        'Productos', 'Total ($ARS)', 'Fecha', 'MP Preference ID',
      ]);
    }
    ...
    // No falla el pago si Sheets falla — solo loguea
